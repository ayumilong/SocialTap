...
define(['dojo/_base/declare',
	...
	return declare([BaseVis], {
		...
		loadData: function(baseQuery) {
			...
			})).then(lang.hitch(this, function(hashtagData) {

				var hashtags = hashtagData.facets.hashtags.terms.map(function(t) { return t.term; });

				var query = lang.clone(baseQuery);
				query.facets = {};
				query.size = 0;
				hashtags.forEach(function(ht) {
					query.facets.times = { date_histogram: { field: 'postedTime', interval: '1h' } };
					query.facets['#' + ht] = {
						date_histogram: { field: 'postedTime', interval: '1h' },
						facet_filter: { term: { 'twitter_entities.hashtags.text': ht } }
					};
				});

				return this.queryDataset(query).then(function(countData) {
					var buckets = countData.facets.times.entries.map(function(e) {
						return {
							date: new Date(e.time),
							count: e.count
						};
					});
					var tagData = hashtags.map(function(ht) {
						return {
							tag: ht,
							values: countData.facets.times.entries.map(function(t) {
								var entry = countData.facets['#' + ht].entries.filter(function(e) { return e.time == t.time; })[0];
		...
		draw: function(data) {
			...
			var x = d3.time.scale().domain(d3.extent(data.buckets.map(function(b) { return b.date; }))).range([0, contentBox.w - 200]);
			var maxCount = d3.max(data.tags.map(function(d) {
			...
			rect.style('fill', 'none')
				...
				.on('mousemove', lang.hitch(this, function() {
					...
					var i = bisectDate(data.buckets, dm, 1);
					var d0 = data.buckets[i - 1].date;
					var d1 = data.buckets[i] && data.buckets[i].date;
					...
					var message = markerDate.toLocaleDateString() + ' ' + markerDate.toLocaleTimeString() + '<br>' +
						'Total: ' + data.buckets[markerIndex].count + '<br><br>';

					message += data.tags.map(function(d, i) {
